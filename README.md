...
> **最終更新**: 2025-12-25 - デバッグモーダル機能を追加（ボタン設定・テンプレート実装状況の確認）
...
### 10. **WinActor風グループ化機能**（NEW! 2025-12-24）
- 複数ノードを選択して右クリック→「グループ化」でグループ作成
- グループに背景オーバーレイを表示
- グループ単位での一括移動が可能
- グループの折りたたみ/展開機能
- WinActorライクな直感的な操作感

### 11. **ヘルプモーダル**（NEW! 2025-12-24）
- メニューの「❓ ヘルプ」ボタンから使い方を確認
- 基本操作、コード生成、関数機能、変数機能の説明
- ノードの色の意味を詳しく解説
- ショートカットキーの一覧

### 12. **ノード色分け機能**（NEW! 2025-12-24）
- 動作タイプ別に自動的にノードを色分け
  - **薄オレンジ（ActionOrange）**: アクション系（外部に変化を与える操作）
  - **薄シアン（ReadCyan）**: 読み込み・取得系（外部から情報を取得）
  - **白（White）**: その他（ダイアログ、内部処理、待機など）
- 特殊ノードの色も維持（条件分岐、ループ、関数など）

### 13. **関数エディタ**（NEW! 2025-12-24）
- 左パネル「関数」タブから関数を管理
- 関数の編集・複製・検索機能
- 関数内にノードを追加・削除可能
- 引数設定ダイアログ付きノード追加

### 14. **Excel接続タブ**（NEW! 2025-12-24）
- 左パネル「接続」タブからExcelファイルに直接接続
- シート選択、変数名設定
- 接続情報（行数、列数、ヘッダー）を表示
- 接続情報の永続化（フォルダ切替時も維持）

### 15. **ロボットタブ**（NEW! 2025-12-24）
- 左パネル「ロボット」タブでワークフローのプロファイルを管理
- ロボット画像・背景色の設定
- 名前、作成者、役割、メモの管理
- 実行オプション（音声あり、表示あり）の設定

### 16. **道具箱メニュー**（NEW! 2025-12-24）
- メニューバーに「🧰 道具箱」ドロップダウンを追加
- 変数管理、フォルダ切替、スナップショット、復元を一箇所に集約
...
1. **グループ作成**
   - SHIFTキーを押しながら複数ノードを選択（赤枠で選択される）
   - 選択したノードを右クリック → 「📁 グループ化」を選択
   - グループ化されたノードには背景オーバーレイが表示される

2. **グループの移動**
   - グループ内のノードをドラッグすると、グループ全体が一括移動

3. **グループの折りたたみ/展開**
   - グループ内のノードを右クリック → 「🔽 グループ折りたたみ/展開」
   - 折りたたむとグループが1つのノードとして表示され、見やすくなる

4. **グループ解除**
   - グループ内のノードを右クリック → 「📂 グループ解除」
...
├── 実行_pode版.bat                         # 起動バッチ
├── 実行_pode版_ノーコンソール.bat           # 起動バッチ（コンソール非表示）
├── ゾーン情報削除.bat                       # ダウンロードブロック解除用
├── ボタン設定.json                        # ノード定義（30種類）
├── カテゴリ設定.json                      # カテゴリ定義（動的生成用）
├── win32API.psm1                          # Windows RPA用ユーティリティモジュール
├── SPEC.md                               # システム仕様書
├── README.md                             # このファイル
├── docs/                                 # ドキュメント
...
### 2025-12-24
- **WinActor風グループ化機能実装**（NEW! 重要機能）
  - 複数ノードを選択して右クリック→「グループ化」でグループ作成
  - グループに背景オーバーレイを表示
  - グループ単位での一括移動機能
  - グループの折りたたみ/展開機能
  - グループ折りたたみ時のノード配置を最適化
- **ヘルプモーダル追加**
  - メニューに「❓ ヘルプ」ボタンを追加
  - 基本操作、コード生成、関数機能、変数機能の使い方を表示
  - ノードの色の意味を詳しく解説
- **ノード色分け機能**
  - 動作タイプ別にノードを自動色分け
  - 薄オレンジ（ActionOrange）: アクション系
  - 薄シアン（ReadCyan）: 読み込み・取得系
  - 白: その他（ダイアログ、内部処理など）
- **関数エディタの大幅改善**
  - 関数の編集・複製・検索機能を追加
  - 関数内へのノード追加・削除機能
  - スクリプト編集ダイアログのサイズを拡大（800×600→1000×700）
  - 関数編集時にボード上の関数ノードも同期更新
- **Excel接続機能**
  - 左パネル「接続」タブにExcel接続機能を実装
  - Excel接続情報の永続化機能を追加
- **ロボットタブ追加**
  - ワークフローのプロファイル管理（名前、作成者、役割、メモ）
  - ロボット画像・背景色のカスタマイズ
  - 実行オプション（音声あり、表示あり）
- **道具箱メニュー**
  - メニューバーに「🧰 道具箱」ドロップダウンを追加
  - 変数管理、フォルダ切替、スナップショット、復元を集約
- **その他UI改善**
  - ファビコン（robo.png）をブラウザタブに追加
  - ノーコンソール版起動バッチ（`実行_pode版_ノーコンソール.bat`）を追加
  - ゾーン情報削除バッチ（`ゾーン情報削除.bat`）を追加
  - フラットデザインへの移行
  - ノードの高さを60%に縮小（40px → 24px）
  - カテゴリ背景色をノード色と競合しないよう調整

### 2025-12-17
- **カテゴリ動的生成システム実装**
  - `カテゴリ設定.json` からカテゴリUI（ボタン・パネル）を動的生成
  - HTMLハードコードを廃止、JSON編集のみでカテゴリ追加可能に
  - `/category-settings.json` APIエンドポイント追加
- **新規ノード11個追加**（win32API.psm1対応）
  - ウインドウ操作: 待機、アクティブ化、最大化、最小化
  - マウス操作: 左クリック、画面中央移動
  - キーボード操作: 文字列貼付、キーワード検索
  - データ処理: クリップボード取得/格納
  - スクリプト実行: デバッグ表示
- **win32API.psm1追加**
  - Windows RPA用ユーティリティモジュール
  - ウインドウ操作、マウス/キーボード操作、UIAutomation対応
- **開発者向けドキュメント追加**
  - `docs/カテゴリ・ノード追加手順.md` - 機能追加手順書
- **PS1ファイル改善**
  - 全ファイルUTF-8 BOM統一
  - ウインドウ系ノードをリスト選択形式に変更
  - 汎用ダイアログ（文字列を入力/数値を入力）形式に統一
