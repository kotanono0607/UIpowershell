...
function 複数行削除 {
    ...
    # 降順でソートして後ろから削除
    $ソート済み = $行番号リスト | Sort-Object -Descending

    foreach ($行番号 in $ソート済み) {
        if ($行番号 -ge 0 -and $行番号 -lt $データ.Count) {
...
function 配列ソート {
    ...
    # ヘッダーを保持
    $ヘッダー = $データ[0]
    $本体 = $データ[1..($データ.Count - 1)]

    # ソート
    $ソート済み = $本体 | Sort-Object {
        $値 = $_[$列番号]
        switch ($データ型) {
            "数値" { [double]$値 }
            "日付" { [datetime]$値 }
...
function 配列フィルタ {
    param(
        ...
        [Parameter(Mandatory=$true)]
        [string]$列名,
        [ValidateSet("完全一致", "部分一致", "前方一致", "後方一致", "より大きい", "以上", "より小さい", "以下", "等しくない")]
        [string]$条件 = "完全一致",
        [Parameter(Mandatory=$true)]
        [string]$値
    )
    ...
    for ($行 = 1; $行 -lt $データ.Count; $行++) {
        ...
        switch ($条件) {
            "完全一致" { $一致 = ($セル値文字列 -eq $値) }
            "部分一致" { $一致 = ($セル値文字列 -like "*$値*") }
            "前方一致" { $一致 = ($セル値文字列 -like "$値*") }
            "後方一致" { $一致 = ($セル値文字列 -like "*$値") }
            "より大きい" {
                try { $一致 = ([double]$セル値 -gt [double]$値) } catch { $一致 = $false }
            }
            "以上" {
                try { $一致 = ([double]$セル値 -ge [double]$値) } catch { $一致 = $false }
            }
            "より小さい" {
                try { $一致 = ([double]$セル値 -lt [double]$値) } catch { $一致 = $false }
            }
            "以下" {
                try { $一致 = ([double]$セル値 -le [double]$値) } catch { $一致 = $false }
            }
            "等しくない" { $一致 = ($セル値文字列 -ne $値) }
        }
