...
| UI要素 | プロパティ | 現在値 | ファイル | 行番号 | 説明 |
|--------|------------|--------|----------|--------|------|
| `#main-container` | `height` | `calc(100vh - 78px + 40px)` | `style-legacy.css` | 208 | メインエリア高さ ⭐重要 |
| `#main-container` | `margin-top` | `-40px` | `style-legacy.css` | 214 | **上マージン（パンくずリストの元スペースを埋める）** ⭐重要 |
| `#main-container` | `padding` | `8px 0` | `style-legacy.css` | 212 | **メインエリア内側余白（上下8px、左右0）ドリルダウンパネル縦長化のため変更** ⭐重要 |
| `#main-container` | `gap` | `12px` | `style-legacy.css` | 213 | 左パネルと中央コンテナの間隔 |
...
| UI要素 | プロパティ | 現在値 | ファイル | 行番号 | 説明 |
|--------|------------|--------|----------|--------|------|
| `#center-container` | `flex` | `1` | `style-legacy.css` | 349 | 残りスペース全て使用 |
| `#center-container` | `min-width` | `640px` | `style-legacy.css` | 350 | 最小幅 |
| `#center-container` | `padding` | `12px 12px 12px 216px` | `style-legacy.css` | 359 | **内側余白（上下12px、左側216px）ドリルダウンパネル縦長化のため変更** ⭐重要 |
| `#center-container` | `gap` | `12px` | `style-legacy.css` | 358 | レイヤーパネル間の間隔 |
| `#center-container` | `border-radius` | `20px` | `style-legacy.css` | 357 | 角丸 |

**計算式の意味**:
- `padding: 12px 12px 12px 216px` = 上(12px) 右(12px) 下(12px) 左(216px)
- 左側padding: `216px` = 元の16px + 200px（パンくずリスト配置スペース）
- 上下padding: `12px` = 16pxから12pxに変更（ドリルダウンパネル縦長化のため）
...
|--------|------------|--------|----------|--------|------|
| **【レイアウト設定】** | | | | | |
| `.drilldown-panel` | `display` | `flex` | `style-legacy.css` | 952 | **Flexboxコンテナ化** ⭐重要 |
| `.drilldown-panel` | `flex-direction` | `column` | `style-legacy.css` | 953 | **縦方向レイアウト** ⭐重要 |
| **【サイズ設定】** | | | | | |
| `.drilldown-panel` | `width` | `300px` | `style-legacy.css` | 956 | **パネル幅（レイヤーパネルと同じ）** ⭐重要 |
| `.drilldown-panel` | `height` | `calc(100vh - 78px)` | `style-legacy.css` | 957 | **高さを画面いっぱいに設定** ⭐重要 |
| `.drilldown-panel` | `min-height` | `1000px` | `style-legacy.css` | 958 | **最小高さ（ノード配置スペースを縦に長く）** ⭐重要 |
| `.drilldown-panel` | `max-height` | `calc(100vh - 20px)` | `style-legacy.css` | 959 | **最大高さ（画面下まで広く）** ⭐重要 |
| **【位置・配置設定】** | | | | | |
| `.drilldown-panel` | `flex-shrink` | `0` | `style-legacy.css` | 962 | 縮小しない設定 |
| `.drilldown-panel` | `position` | `relative` | `style-legacy.css` | 963 | 位置指定方法 |
| `.drilldown-panel` | `margin-left` | `12px` | `style-legacy.css` | 964 | **左マージン（レイヤーパネルとの間隔）** ⭐重要 |
| `.drilldown-panel` | `margin-top` | `0px` | `style-legacy.css` | 965 | **上マージン** |
| `.drilldown-panel` | `margin-bottom` | `0px` | `style-legacy.css` | 966 | **下マージン** |
| **【内枠設定（ノード配置スペース）】** | | | | | |
| `.drilldown-panel` | `padding` | `12px` | `style-legacy.css` | 969 | **★内側余白（内枠）★ この数値を変更** ⭐最重要 |
| | | | | | **実質的な内側幅 = 300px - 12px × 2 = 276px** |
| | | | | | **実質的な内側高さ = 1000px - 12px × 2 = 976px（最小）** |
| | | | | | **変更例: 8px→284px（幅）/984px（高さ）、16px→268px（幅）/968px（高さ）** |
| **【表示設定】** | | | | | |
| `.drilldown-panel` | `overflow-y` | `auto` | `style-legacy.css` | 975 | 縦スクロール設定 |
| `.drilldown-panel` | `border-radius` | `20px` | `style-legacy.css` | 976 | 角丸 |
| `.drilldown-panel` | `box-shadow` | 複数値 | `style-legacy.css` | 979-981 | ニューモーフィズム影 |
| **【ノード配置枠】** | | | | | |
| `.drilldown-panel .node-list-container` | `flex` | `1` | `style-legacy.css` | 994 | **親の残りスペースを全て使う** ⭐最重要 |
| `.drilldown-panel .node-list-container` | `min-height` | `unset` | `style-legacy.css` | 995 | **固定高さ制限を解除** ⭐重要 |
| `.drilldown-panel .node-list-container` | `overflow-y` | `auto` | `style-legacy.css` | 996 | 縦スクロール設定 |
| **【編集ボタン】** | | | | | |
| `.drilldown-edit-btn` | `padding` | `5px 12px` | `style-legacy.css` | 1000 | 編集ボタン余白 |
| `.drilldown-edit-btn` | `font-size` | `13px` | `style-legacy.css` | 1006 | 編集ボタン文字サイズ |
| `.drilldown-edit-btn` | `border-radius` | `6px` | `style-legacy.css` | 1002 | 編集ボタン角丸 |

**重要な注意事項**:
- **★Flexboxレイアウト（2025-11-15追加）★**: `.drilldown-panel`は`display: flex; flex-direction: column;`でFlexboxコンテナ化されています
  - **内側のノード配置枠（`.node-list-container`）が`flex: 1`で親の残りスペースを全て使います** ⭐最重要
  - これにより、ノード配置枠が**画面下まで自動的に伸びる**ようになりました
  - JavaScriptの`min-height: 400px`制限を削除し、CSS側で`min-height: unset`に設定
- **★内枠（ノード配置スペース）の調整★**: **969行目の `padding` を変更**することで内側の大きさを数値で調整できます
...
- **サイズ設定**: 横幅（`width`）、高さ（`height`）、最小高さ（`min-height`）、最大高さ（`max-height`）を数値で調整可能
  - 高さ: `height: calc(100vh - 78px)`（画面いっぱい）
  - 最小高さ: `min-height: 1000px`（ノード配置スペースを縦に長く確保）
  - 最大高さ: `calc(100vh - 20px)` = 画面高さ100% - ヘッダー等の20px
- **縦長デザイン**: 幅300px、高さ画面いっぱい、内側余白12pxで縦長のデザイン
- **位置設定**: 上下左右のマージン（`margin-top/bottom/left`）で表示位置を調整可能
- **レイヤーパネルとの間隔**: `margin-left: 12px`（コンパクトに近づけた）
- **空状態**: `.drilldown-panel.empty` クラスで非表示時は透明背景になります（984-990行目）
...
.drilldown-panel {
    height: calc(100vh - 60px);  /* 高さを調整（-78px → -60px） */
    min-height: 1200px;  /* 1000px → 1200px（さらに縦に長く） */
    max-height: calc(100vh - 40px);  /* 最大高さをさらに広げる */
}

/* ========================================
   【ノード配置枠の調整】（2025-11-15追加）
   ======================================== */

/* ノード配置枠の高さはflex: 1で自動調整されます */
/* 固定の高さ制限を追加したい場合 */
.drilldown-panel .node-list-container {
    flex: 1;
    min-height: 800px;  /* 最小高さを設定 */
...
│    └─ button (32px高さ)                                       │
├──────────────────────────────────────────────────────────────┤
│ #main-container (margin-top: -40px, padding: 8px 0, gap: 12px)│
│ ┌────────────────────┬──┬────────────────────────────────────┤
...
| 日付 | 変更内容 | 理由 |
|------|----------|------|
| 2025-11-15 | **ドリルダウンパネル内のノード配置枠を画面下まで拡張** (`.drilldown-panel`に`display: flex; flex-direction: column;`追加、`.drilldown-panel .node-list-container`に`flex: 1; min-height: unset;`設定、JavaScriptの`min-height: 400px`削除) | Flexboxレイアウトでノード配置枠が親の残りスペースを全て使い、画面下まで自動的に伸びるように改善。固定高さ制限を全て解除 |
| 2025-11-15 | **親要素のpaddingを調整してドリルダウンパネルの実質高さを拡張** (`#main-container` padding `36px→8px`、`#center-container` padding `16px→12px`) | 親要素のpaddingが大きすぎてドリルダウンパネルの高さが制限されていた問題を解決。合計64px分縦に広がる |
