@echo off
rem Ver1.2 フォルダ配下のゾーン情報 点検→削除→再点検（PowerShell一括・Bypass、集計修正）
setlocal EnableExtensions

rem --- フォルダ選択ダイアログ ---
for /f "usebackq delims=" %%I in (`powershell -ExecutionPolicy Bypass -NoProfile -Command ^
  "Add-Type -AssemblyName System.Windows.Forms; $d=New-Object Windows.Forms.FolderBrowserDialog; $d.Description='ゾーン削除対象のフォルダを選択'; $d.ShowNewFolderButton=$false; if($d.ShowDialog() -eq 'OK'){ $d.SelectedPath }"`) do set "TARGET=%%I"
