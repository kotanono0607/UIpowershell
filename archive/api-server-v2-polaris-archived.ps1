...
# ノードをコピー（新しいIDで複製）
New-PolarisRoute -Path "/api/nodes/copy" -Method POST -ScriptBlock {
    Set-CorsHeaders -Response $Response
    try {
        ...
        try {
            ...
            if ($元のコード) {
                ...
                $コード追加結果 = エントリを追加_指定ID -ID $newNodeId -Code $元のコード
                if ($コード追加結果.success) {
    ...
    try {
        ...
        if (Test-Path $memoryPath) {
            try {
                ...
                Write-Host "[履歴] memory.json保存前の状態を取得しました" -ForegroundColor Gray
            } catch {
                Write-Host "[履歴] memory.json読み込みエラー（新規作成の可能性あり）" -ForegroundColor DarkGray
            }
        } else {
            Write-Host "[履歴] memory.jsonが存在しません（新規作成）" -ForegroundColor DarkGray
        }
        ...
        try {
            ...
            Write-Host "[履歴] 操作を記録しました" -ForegroundColor Green
        } catch {
