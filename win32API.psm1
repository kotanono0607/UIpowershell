...
# ========================================
# Excel操作関数（ImportExcelモジュール使用）
# 追加: 2025-12-22 - output.ps1実行時にExcel操作を可能にするため
# ========================================
    ...
    try {
        ...
        if ($colorValue.IsEmpty -or $colorValue.A -eq 0) {
            if ($色 -match "^#[0-9A-Fa-f]{6}$") {
                ...
                $colorValue = [System.Drawing.Color]::Yellow
            }
        }
...
Export-ModuleMember -Function 画像待機, 画像存在確認, 画像クリック, スクリーンショット保存, ウィンドウスクリーンショット, 入力ダイアログ表示, `
    Ensure-ImportExcelModule, 列番号を列文字に変換, 列文字を列番号に変換, セルアドレス生成, `
    Excel操作_セル値取得, Excel操作_セル値設定, Excel操作_セル値取得_行列, Excel操作_セル値設定_行列, `
    Excel操作_背景色設定, Excel操作_フォント設定, Excel操作_罫線設定, Excel操作_数式設定, `
    Excel操作_シート追加, Excel操作_シート名変更, Excel操作_シート削除, Excel操作_範囲クリア, `
    Excel操作_最終行取得, Excel操作_最終列取得, Excel操作_行挿入, Excel操作_行削除, `
    Excel操作_列幅自動調整, Excel操作_シート一覧取得
