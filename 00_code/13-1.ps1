function 13_1 {
    ...
    $フォーム.Controls.AddRange(@($ラベル1, $chkPathVar, $パステキスト, $cmbPathVar, $参照ボタン, $ラベル2, $シートテキスト, $ラベル3, $chkColVar, $列テキスト, $cmbColVar, $ラベル4, $chkRowVar, $行テキスト, $cmbRowVar, $ラベル5, $変数テキスト, $ヒントラベル, $OKボタン, $キャンセルボタン))
    ...
    $entryString = @"
# セル値取得: $列表示, $行表示
`$取得値 = Excel操作_セル値取得_行列 -ファイルパス $パスパラメータ -列番号 $列パラメータ -行番号 $行パラメータ$シートパラメータ
