...
function 8_11 {
    ...
    $格納先テキスト.Location = New-Object System.Drawing.Point(20, 145)
    ...
    $フォーム.Controls.AddRange(@($ラベル1, $変数コンボ, $ヘッダーチェック, $ラベル2, $格納先テキスト, $OKボタン, $キャンセルボタン))
    ...
    $ヘッダー除外 = $ヘッダーチェック.Checked
    $格納先 = $格納先テキスト.Text
    $除外オプション = if ($ヘッダー除外) { "-ヘッダー除外" } else { "" }

    $entryString = @"
# データ件数取得: $選択変数
`$データ = 変数の値を取得する -変数 `$変数 -名前 "$選択変数"
`$$格納先 = データ件数 -データ `$データ $除外オプション
変数を追加する -変数 `$変数 -名前 "$格納先" -型 "単一値" -値 `$$格納先
Write-Host "$選択変数 のデータ件数: `$$格納先 件"
"@
