﻿function 13_6 {
    ...
    $フォーム.Text = "数式設定"
    $フォーム.Size = New-Object System.Drawing.Size(520, 360)
    ...
    $ラベル4.Text = "数式（例: =SUM(A1:B1)）："
    $ラベル4.Location = New-Object System.Drawing.Point(20, 215)
    $ラベル4.AutoSize = $true

    $数式テキスト = New-Object System.Windows.Forms.TextBox
    $数式テキスト.Location = New-Object System.Drawing.Point(20, 240)
    $数式テキスト.Size = New-Object System.Drawing.Size(250, 25)
    $数式テキスト.Text = "=SUM(A1:B1)"

    # ボタン
    $OKボタン = New-Object System.Windows.Forms.Button
    $OKボタン.Text = "OK"
    $OKボタン.Location = New-Object System.Drawing.Point(300, 280)
    $OKボタン.Size = New-Object System.Drawing.Size(90, 30)
    $OKボタン.DialogResult = [System.Windows.Forms.DialogResult]::OK

    $キャンセルボタン = New-Object System.Windows.Forms.Button
    $キャンセルボタン.Text = "キャンセル"
    $キャンセルボタン.Location = New-Object System.Drawing.Point(400, 280)
    $キャンセルボタン.Size = New-Object System.Drawing.Size(90, 30)
    $キャンセルボタン.DialogResult = [System.Windows.Forms.DialogResult]::Cancel

    $フォーム.Controls.AddRange(@($ラベル1, $chkPathVar, $パステキスト, $cmbPathVar, $参照ボタン, $ラベル2, $シートテキスト, $ラベル3, $セルテキスト, $ラベル4, $数式テキスト, $OKボタン, $キャンセルボタン))
    ...
    if ([string]::IsNullOrWhiteSpace($ファイルパス) -or [string]::IsNullOrWhiteSpace($セルアドレス) -or [string]::IsNullOrWhiteSpace($数式)) {
        [System.Windows.Forms.MessageBox]::Show("ファイルパス、セルアドレス、数式は必須です。", "エラー")
        return $null
    }
    ...
    $entryString = @"
# 数式設定: $セルアドレス = $数式
Excel操作_数式設定 -ファイルパス $パスパラメータ -セルアドレス "$セルアドレス" -数式 "$数式"$シートパラメータ
"@
