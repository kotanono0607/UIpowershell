...
# デバッグ出力：取得した配列の構造確認
Write-Host "=== 8-1.ps1 デバッグ ==="
Write-Host "myTwoDimArrayの型: $($myTwoDimArray.GetType().FullName)"
Write-Host "myTwoDimArrayの要素数: $($myTwoDimArray.Count)"
if ($myTwoDimArray.Count -gt 0) {
    Write-Host "myTwoDimArray[0]の型: $($myTwoDimArray[0].GetType().FullName)"
    if ($myTwoDimArray[0] -is [System.Array]) {
        Write-Host "myTwoDimArray[0]の要素数: $($myTwoDimArray[0].Count)"
    }
}
Write-Host "========================="
