        ...
        $global:グループモード = 1
            ...
            if ($最後の文字 -ge 1) {
                ...
                if ($Global:Pink選択配列[$現在のレイヤー番号].値 -ne 1) {
                    ...
                    Write-Host "[❌ 右矢印] レイヤー$現在のレイヤー番号 でスクリプト展開中ではないため、進めません" -ForegroundColor Red
                    return  # 処理を中断
                }

                Write-Host "[✅ 右矢印] レイヤー$現在のレイヤー番号 でスクリプト展開中を確認。レイヤー$($現在のレイヤー番号 + 1) に進みます" -ForegroundColor Green
            }
